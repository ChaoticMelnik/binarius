...
describe('POST /auth/binodex/callback', () => {
  ...
  it('refuses a blocked user and leaves them blocked', async () => {
    const blocked = await seedUser(tmp.db, { status: 'blocked' });
    const { response } = await login(blocked.telegramUserId, `broker-${blocked.telegramUserId}`);
    expect(response.statusCode).toBe(409);
