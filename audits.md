...
2. **Каждая фаза проверяет запуском каждый вариант, который предлагает владельцу в clarify-вопросе (флаги, API, возможности инструмента).** Q8 пришлось пересматривать. — **внедрено в #63: `.claude/skills/architect/SKILL.md` → Step 5; `.claude/skills/implementer/SKILL.md` → Step 0; `.claude/skills/tech-lead/SKILL.md` → Phases run as spawned agents, «Clarify relay»**
